...
export default function LogIn() {
  const navigate = useNavigate()
    ...
    return (
      ...
      <div className="flex w-full gap-8 max-w-[1500px] mx-auto  justify-center px-12">
        
            <form  onSubmit={signInUser} className="flex gap-6 flex-col w-full max-w-lg bg-[var(--surface)] shadow-[var(--shadow-lg)] rounded-2xl py-8 px-12">
                <div>
                    <h1 className="mt-4 heading-font text-3xl text-center text-[var(--primary)]">Welcome back</h1>
                    <p className="mt-2 body-font text-center text-sm text-[var(--primary-light)]">Log in to your Pantry account.</p>
                </div>
                <div>
                ...
                <div className="flex flex-col">
                    ...
                    <p className="mt-2 underline text-[var(--accent)]">Forgot your password?</p>
                </div>
                <div className="mt-6">
                    <button type="submit" className="flex w-full items-center justify-center gap-3 rounded-lg border border-[var(--border)] bg-[var(--accent)] px-4 py-3 text-sm font-medium text-white shadow-[var(--shadow-sm)] transition-colors duration-200 hover:bg-[var(--background-soft)] hover:text-[var(--text-primary)] focus:outline-none cursor-pointer">Log In</button>
                </div>
                <p className="text-center">Don't have an account? <NavLink to="/sign-up" className="underline text-[var(--accent)]">Sign up</NavLink></p>
            </form>
            <div className="hidden flex-1 lg:flex w-full max-w-lg">
              <img className="w-full h-full object-cover rounded-2xl" src={LogInImg}/>
            </div>
        </div> 
      </main>
      </div>
